...
public class FacturaXmlBuilder implements DocBuilder {
    ...
    public void buildDoc() {
        ...
        try {
            ...
            var totalStr = (String) ds.get("CFDI_TOTAL");
            String[] a = totalStr.split("\\.");
            ...
            cfdi.setLugarExpedicion((String) ds.get("EMISOR_CP"));
            cfdi.setMetodoPago(CMetodoPago.fromValue((String) ds.get("METODO_PAGO")));
            cfdi.setTipoDeComprobante(CTipoDeComprobante.fromValue("I")); //TODO: hardcoded
            cfdi.setTotal(new BigDecimal(totalStr));
            ...
            cfdi.setSubTotal(new BigDecimal((String) ds.get("CFDI_SUBTOTAL")));
            cfdi.setCondicionesDePago("hardcoded"); //TODO: hardcoded
            cfdi.setNoCertificado((String) ds.get("CFDI_CERT_NO"));
            cfdi.setFormaPago((String) ds.get("FORMA_PAGO"));
            ...
            cfdi.setVersion((String) ds.get("TVER"));

            cfdi.setFecha(DatatypeFactory.newInstance().newXMLGregorianCalendar((String) ds.get("CFDI_DATE")));

            // Emisor
            var emisor = cfdiFactory.createComprobanteEmisor();
            emisor.setRfc((String) ds.get("EMISOR_RFC"));
            emisor.setNombre((String) ds.get("EMISOR_NOMBRE"));
            emisor.setRegimenFiscal((String) ds.get("EMISOR_REG"));
            cfdi.setEmisor(emisor);

            // Receptor
            var receptor = cfdiFactory.createComprobanteReceptor();
            receptor.setRfc((String) ds.get("RECEPTOR_RFC"));
            receptor.setNombre((String) ds.get("RECEPTOR_NOMBRE"));
            receptor.setUsoCFDI(CUsoCFDI.fromValue((String) ds.get("RECEPTOR_USO")));
            ...
            var complementoList = complemento.getAny();
            var cartaPorteFactory = new mx.gob.sat.cartaporte.ObjectFactory();
            CartaPorte cartaPorte = cartaPorteFactory.createCartaPorte();
            complementoList.add(cartaPorte);
